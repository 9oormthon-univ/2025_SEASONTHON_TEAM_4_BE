...
from app.utils.quest_utils import generate_llm_quests, select_quests_from_pool
...
from data.quest_templates import RECORD_QUEST_POOL
...
@app.route("/parent/analyze", methods=["GET"])
def glucose_analyze():
    """혈당 종합 분석 및 비서 코칭"""
    try:
        member_id = int(request.args.get("member_id", 1))
        ...
        # AI 분석 시도
        prompt = load_text("prompts/parent_analyze_prompt.txt")
...
@app.route("/child/request", methods=["POST"])
...
@app.route("/parent/approve", methods=["POST"])
...
@app.route("/parent/report", methods=["GET"])
def glucose_summary_api():
    ...
    try:
        ...
        prompt_text = load_text("prompts/parent_report_prompt.txt")
...
@app.route("/child/report", methods=["GET"])
def child_report_api():
    ...
    try:
        ...
        formatted_data = format_glucose_data(glucose_data)
        glucose_metrics = calculate_glucose_metrics(formatted_data)
        prompt_text = load_text("prompts/child_report_prompt.txt")
        analysis_result = analyze_glucose(glucose_metrics, prompt_text, member_info.get('age'))
